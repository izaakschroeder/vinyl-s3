...
var includes = _.includes || _.contains;
...
module.exports = function createWriteStream(root, options) {
	...
	assignWith(awsOptions, _.isString(root) ? url.urlToOptions(root) : root);
	...
	return through2.obj(function writeStream(file, encoding, callback) {
		...
		if (contentEncoding.length > 0 && !contentType) {
			var last = _.chain(name.split('.'))
				.dropRightWhile(function checkPart(ext) {
					return includes(contentEncoding, encodings[ext]);
				})
				.last()
				.value();
		...
		var fileOptions = assignWith({ }, awsOptions, {
		...
		if (!file.isNull()) {
			...
			s3.upload(assignWith(fileOptions, {
				Body: file.contents
